...
- [x] T001 Create `go.work` and `go.work.sum` at repo root listing `rook-cli/` and `rook-server/` modules
- [x] T002 [P] Create `rook-cli/go.mod` with module path `github.com/rook-project/rook-reference/rook-cli` and `go 1.23` directive
- [x] T003 [P] Create `rook-server/go.mod` with module path `github.com/rook-project/rook-reference/rook-server` and `go 1.23` directive
- [x] T004 [P] Create root `.golangci.yml` enabling linters: `gofmt`, `govet`, `errcheck`, `staticcheck`
- [x] T005 [P] Create `.github/workflows/` directory structure (empty placeholder files for workflow files added in T020–T022)
- [x] T006 Create root `Makefile` with targets: `build` (delegates to both modules), `test`, `lint`, `clean`; version injected via `git describe --tags --always` falling back to `dev`
- [x] T007 [P] Create `rook-cli/Makefile` with targets: `build` (output to `../../dist/rook-cli`), `test`, `lint`
- [x] T008 [P] Create `rook-server/Makefile` with targets: `build` (output to `../../dist/rook-server-cli`), `test`, `lint`
...
- [x] T014 [US1] Create `rook-cli/main.go` — parses `--version` / `-v` (prints `rook version <version>`, exits 0) and `--help` / `-h` (prints usage including config path, exits 0); calls `config.Load(config.ConfigPath())`; on `ErrNotFound` prints a single-line notice and exits 0; on other errors prints to stderr and exits 1
- [x] T015 [P] [US1] Create `rook-server/cmd/admin/main.go` — stub: `var version string`; prints `rook-server-cli version <v>` (zero value → `dev`) and exits 0; accepts `--version` / `-v` and `--help` / `-h`
- [x] T016 [US1] Run `go work sync` and verify `go build ./...` succeeds in both modules (update `go.work.sum` as needed)
- [x] T017 [US1] Verify `make build` from repo root produces `dist/rook-cli` and `dist/rook-server-cli`; add `dist/` to `.gitignore`
...
- [x] T018 [US2] Create `.github/workflows/_go-ci.yml` — reusable workflow (`workflow_call`) with `working-directory` input; steps: `actions/checkout@v4`, `actions/setup-go@v5` (version from `go.mod`), `golangci/golangci-lint-action@v6`, `go test ./... -race -count=1`, `go build ./...`
- [x] T019 [P] [US2] Create `.github/workflows/ci-rook-cli.yml` — triggers on `push`/`pull_request` to `main` with `paths: ['rook-cli/**', '.github/workflows/ci-rook-cli.yml', '.github/workflows/_go-ci.yml']`; calls `_go-ci.yml` with `working-directory: rook-cli`
- [x] T020 [P] [US2] Create `.github/workflows/ci-rook-server-cli.yml` — same pattern; `paths: ['rook-server/cmd/admin/**', '.github/workflows/ci-rook-server-cli.yml', '.github/workflows/_go-ci.yml']`; `working-directory: rook-server`
- [x] T021 [US2] Document branch protection setup in `rook-cli/README.md`: enable "Require status checks to pass" on `main` for `ci-rook-cli / ci` and `ci-rook-server-cli / ci`
...
- [x] T022 [US3] Add `config.Validate(cfg Config) error` to `rook-cli/internal/config/config.go` — validates `ServerEntry.Address` is non-empty absolute URL when servers list is non-empty; validates `StorageDir` is absolute path if non-empty; returns nil for empty/zero-value Config
- [x] T023 [US3] Update `rook-cli/main.go` to call `config.Validate()` after `config.Load()`; on validation error, print actionable message to stderr and exit 1
- [x] T024 [US3] Add test cases to `rook-cli/internal/config/config_test.go` for `Validate`: valid config passes; empty Address in ServerEntry fails; relative StorageDir fails; unknown FeatureFlags keys round-trip without loss
- [x] T025 [P] [US3] Document config path resolution and env-var overrides in `rook-cli/README.md` config section (XDG table, `ROOK_CONFIG_PATH`, `XDG_CONFIG_HOME`)
...
- [x] T026 [P] Create `rook-cli/README.md` — sections: Overview, Prerequisites, Clone & Build, Test, Lint, Config Paths (XDG table), Per-Module Commands, Troubleshooting (from quickstart.md)
- [x] T027 [P] Create root `README.md` — project overview, monorepo layout, quick-start (`make build`), link to `rook-cli/README.md` and `specs/`
- [x] T028 [P] Create `CONTRIBUTING.md` at repo root — Go version pinning note, `go work sync` requirement, Makefile targets reference, PR process, branch naming (`NNN-feature-name`)
- [x] T029 Add `.gitignore` entries: `dist/`, `*.test`, `*.out`, coverage files
- [x] T030 [P] Run `make lint && make test && make build` from repo root and confirm all pass; fix any issues discovered
